...
const ReportStub: React.FC<PaycheckProps> = ({ paycheckData }) => {
  ...
  // Laptop purchase data from attachment
  const laptopData = {
    reference: "BRDINV24020224142",
    paymentType: "Laptop Lenovo,Notebook,I3,8GB RAM,512GB SSD",
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-lg overflow-hidden">
      ...
      <div className="p-6 border-b border-gray-300">
        <h3 className="text-lg font-bold mb-3 text-blue-800">Laptop Purchase Details</h3>
        <div className="bg-white p-4 rounded shadow">
          <div className="grid grid-cols-2 gap-4 mb-3">
            <div>
              <p><span className="font-semibold">Reference:</span> {laptopData.reference}</p>
              <p><span className="font-semibold">Payment Type:</span> {laptopData.paymentType}</p>
              <p><span className="font-semibold">Payment Period:</span> {laptopData.paymentPeriod}</p>
              <p><span className="font-semibold">Amount:</span> {laptopData.amount}</p>
            </div>
            <div>
              <p><span className="font-semibold">Payment Channel:</span> {laptopData.paymentChannel}</p>
              <p><span className="font-semibold">Date:</span> {laptopData.date}</p>
              <p><span className="font-semibold">Status:</span> {laptopData.status}</p>
            </div>
          </div>
        </div>
      </div>

      {/* Equipment Specifications - New section based on attachment */}
      <div className="p-6 border-b border-gray-300">
        <h3 className="text-lg font-bold mb-3 text-blue-800">Equipment Specifications</h3>
        <div className="bg-white p-4 rounded shadow">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p><span className="font-semibold">Brand:</span> Lenovo</p>
              <p><span className="font-semibold">Type:</span> Notebook</p>
            </div>
            <div>
              <p><span className="font-semibold">Processor:</span> Intel Core i3</p>
              <p><span className="font-semibold">Memory:</span> 8GB RAM</p>
              <p><span className="font-semibold">Storage:</span> 512GB SSD</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800 text-white p-6 text-sm">
        <div className="grid grid-cols-2">
          <div>
            <p>Report ID: PR-{laptopData.reference}</p>
