...
import { generateEmployeeTemplate } from "@/actions/templatesActions";
...
export default function MultipleUploadForm() {
  const handleDownloadTemplate = async () => {
    try {
      const base64 = await generateEmployeeTemplate();
  ...
  const onSubmit = async (data: FormInputs) => {
    ...
    startTransition(async()=>{
     ...
     try {
      ...
      const result = await processExcelFile( {formData:formData});
