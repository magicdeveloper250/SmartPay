...
export function ContractorPayrollDetail({ mainPayroll, payrolls }: { mainPayroll: MainPayroll, payrolls: PayrollWithContractor[] }){
  ...
  const filteredPayrolls = payrolls?.filter(payroll => {
    const matchesSearch = (payroll?.contractor?.firstName ?? '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                         (payroll?.contractor?.secondName ?? '').toLowerCase().includes(searchTerm.toLowerCase());
