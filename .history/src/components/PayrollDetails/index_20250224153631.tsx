...
import { MainPayroll, Payroll, PayrollType, Contractor } from '@prisma/client';
...
// Update the type to handle both employee and contractor
type PayrollWithPerson = {
  id: string;
...
export function PayrollDetail ({ mainPayroll, payrolls, payrollType }: { mainPayroll: MainPayroll, payrolls: payrollWithEmployee[], payrollType: PayrollType }){
  ...
  const isContractor = mainPayroll.payrollType === PayrollType.CONTRACTOR;
  ...
  const PayButton = ({ status, payrollId }: { status: PaymentStatus; payrollId: string }) => {
    ...
    const baseButtonClasses = "px-4 py-2 text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors duration-200 inline-flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed";
        ...
        return (
          <div className="flex items-center gap-2">
            <button
              ...
              className={`${baseButtonClasses} text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-500`}
            >
              {isLoading ? (
            ...
            <button
              ...
              className={`${baseButtonClasses} text-white bg-red-600 hover:bg-red-700 focus:ring-red-500`}
            >
              {isLoading ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
      ...
      case PaymentStatus.Paid:
        return (
          <button
            ...
            className={`${baseButtonClasses} text-gray-700 bg-gray-100 cursor-not-allowed`}
          >
            <CheckCircle2 className="w-4 h-4" />
            Paid
          </button>
      ...
      case PaymentStatus.Cancelled:
        return (
          <button
            ...
            disabled={isLoading}
            aria-label="Activate cancelled payment"
            className={`${baseButtonClasses} text-white bg-green-600 hover:bg-green-700 focus:ring-green-500`}
          >
            {isLoading ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
  ...
  const renderPersonDetails = (payroll: PayrollWithPerson) => {
    const person = isContractor ? payroll.contractor : payroll.employee;
    return (
      <div>
        <p className="font-medium text-gray-900">
          {person?.firstName} {person?.secondName}
        </p>
        {isContractor && payroll.contractor?.company && (
          <p className="text-sm text-gray-500">{payroll.contractor.company.name}</p>
        )}
      </div>
    );
  };
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <p className="font-medium">{payrolls[0]?.netSalary}</p>
        </div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          <p className="text-sm text-gray-600 mb-2">
            {isContractor ? 'Total Contractors' : 'Total Employees'}
          </p>
    ...
    return (
      <div className="border rounded-lg overflow-hidden">
        <div className="grid grid-cols-4 bg-gray-50 p-4 border-b">
          ...
          <div className="font-medium text-gray-600">
            {isContractor ? 'Contract Amount' : 'Net Salary'}
          </div>
        ...
        <div className="divide-y">
          {filteredPayrolls.map(payroll => (
            <div key={payroll.id} className="grid grid-cols-4 p-4 items-center hover:bg-gray-50 transition-colors cursor-pointer group">
              {renderPersonDetails(payroll)}
              <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800">
            {isContractor ? 'Contractor Payroll' : 'Employee Payroll'}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-medium">
              {isContractor ? 'Contractors' : 'Employees'}
            </h2>
