...
import { Dialog, DialogBackdrop, DialogPanel, PopoverButton, PopoverPanel, Popover } from '@headlessui/react';
...
export function PayrollDetail({ mainPayroll, payrolls }: { mainPayroll: MainPayroll, payrolls: PayrollWithEmployeeAndCustomTaxes[]}) {
    ...
    switch (status) {
      ...
      case PaymentStatus.Paid:
        return (
          ...
          <button
            onClick={()=>{
              setCurrentPayroll(payrollId)
              setViewPaycheck(true)
            }}
  ...
  <li key={idx} className="grid grid-cols-1 text-sm relative z-10">
    <Popover className="relative">
      {({ open }) => (
        <>
          <PopoverButton className="w-full text-left">
            <div className="flex flex-col">
              <span className="p-2 border-r font-extrabold">
                {tax.taxName}
              </span>
              <span className="p-2 font-medium">
                {new Intl.NumberFormat('en-US', { style: 'currency', currency: "RWF" }).format(tax.taxAmount)}
              </span>
            </div>
          </PopoverButton>
          <PopoverPanel className="absolute z-[9999] w-72 bg-white shadow-lg rounded-md p-4 border mt-2 left-0 isolate">
            <div className="space-y-2">
              <div className="flex justify-between items-center border-b pb-2">
                <h3 className="font-medium">{tax.taxName} Details</h3>
                <button onClick={() => open = false}>
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div dangerouslySetInnerHTML={{__html: tax.taxDescription}} />
            </div>
          </PopoverPanel>
        </>
      )}
   ...
   <PaycheckComponent open={viewPayCheck} paycheckData={currentPayroll!==null ? payrolls.filter(p=>p.id==currentPayroll)[0]:payrolls[0]} onClose={()=>setViewPaycheck(false)}/>
