        ...
        return (
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-white bg-red-800 rounded-md hover:bg-red-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-900 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2"
            >
      ...
      case PaymentStatus.Cancelled:
        return (
          <button
            ...
            disabled={isMainActionLoading}
            className="px-4 py-2 text-sm font-medium text-white bg-green-800 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2"
          >
      ...
      case PaymentStatus.Pending:
        return (
          ...
          <button
            ...
            disabled={isLoading}
            className="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
      ...
      case PaymentStatus.Failed:
        return (
          <button
            onClick={() => handlePay(payrollId)}
            disabled={isLoading}
            className="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-900 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
        ...
        case PaymentStatus.Cancelled:
          return (
            <button
              ...
              disabled={isLoading}
              className="px-4 py-2 text-sm font-medium text-white bg-green-800 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-900 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
            ...
            {filteredPayrolls.map(payroll => (
              ...
              >
                ...
                <div>
                  <div>
                    <Link 
                      href={`/contractor/internal/${payroll?.contractId}/contract`}
                      scroll={false}
                      className="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-primary hover:text-blue-800 bg-blue-50 hover:bg-blue-100 rounded-md transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-1"
                      aria-label={`View contract details for ${payroll?.contractor?.firstName} ${payroll?.contractor?.secondName}`}
                    >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link href={`/contractor/internal/${payroll?.contractor?.id}`} scroll={false}><ChevronRight className="w-5 h-5 text-gray-400" /></Link>
                </div>
              </div>
            ))}
          </div>
