...
export function PayrollDetail ({ mainPayroll, payrolls }: { mainPayroll: MainPayroll, payrolls: payrollWithEmployee[] }){
  ...
  useEffect(() => {
    console.log("Initial props:", { mainPayroll, payrolls });
    if (mainPayroll && payrolls) {
      console.log("Setting loading to false");
      setIsLoading(false);
    }
  }, [mainPayroll, payrolls]);
  ...
  useEffect(() => {
    if (mainPayroll?.id && Array.isArray(payrolls)) {
      console.log("Data is ready:", { 
        mainPayrollId: mainPayroll.id, 
        payrollsLength: payrolls.length 
      });
      setIsLoading(false);
    }
  }, [mainPayroll, payrolls]);
  ...
  console.log("Filtered Payrollls: ", filteredPayrolls)
  ...
  if (isLoading || !mainPayroll?.id || !Array.isArray(payrolls)) {
    return (
