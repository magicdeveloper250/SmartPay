...
import { MainPayroll, Payroll, PayrollType } from '@prisma/client';
...
import { PayrollReturnType, payrollWithEmployee, payrollWithContractor, PayrollWithPerson } from '@/types/payroll';

export function PayrollDetail ({ mainPayroll, payrolls, payrollType }: { mainPayroll: MainPayroll, payrolls:  PayrollWithPerson[], payrollType: PayrollType }){
  ...
  const filteredPayrolls = payrolls?.filter(payroll => {
    ...
    const matchesSearch = (payrollType === PayrollType.EMPLOYEE 
      ? ((payroll as payrollWithEmployee)?.employee?.firstName ?? '').toLowerCase()
      : ((payroll as payrollWithContractor)?.contractor?.firstName ?? '').toLowerCase())
      .includes(searchTerm.toLowerCase()) ||
      (payrollType === PayrollType.EMPLOYEE 
        ? ((payroll as payrollWithEmployee)?.employee?.secondName ?? '').toLowerCase()
        : ((payroll as payrollWithContractor)?.contractor?.secondName ?? '').toLowerCase())
      .includes(searchTerm.toLowerCase());
              ...
              >
                <div>
                  <p className="font-medium text-gray-900">
                    {payrollType === PayrollType.EMPLOYEE 
                      ? `${(payroll as payrollWithEmployee)?.employee?.firstName} ${(payroll as payrollWithEmployee)?.employee?.secondName}`
                      : `${(payroll as payrollWithContractor)?.contractor?.firstName} ${(payroll as payrollWithContractor)?.contractor?.secondName}`}
                  </p>
                </div>
                <div>
