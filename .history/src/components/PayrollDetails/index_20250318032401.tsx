...
export function PayrollDetail({ mainPayroll, payrolls }: { mainPayroll: MainPayroll, payrolls: payrollWithEmployee[] }) {
  ...
  const currentTaxYear = getCurrentSupportedTaxYear(); // Extract the year
  ...
  return (
    ...
    >
      ...
      {isPIT && hoveredTaxIndex === index && (
        <div className="absolute z-10 bg-white p-4 border rounded-lg shadow-lg top-10 left-0 w-64 max-w-[90vw] overflow-auto transform -translate-x-1/2">
          <pre className="text-sm whitespace-pre-wrap">
            {explainTaxCalculation({
              taxRates: getRwandaTaxRates({taxYear:currentTaxYear}),
              taxableAnnualIncome: payroll.salary,
              personalAllowance: 0,
            })}
          </pre>
