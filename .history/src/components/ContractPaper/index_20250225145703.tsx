        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-md border border-gray-100 space-y-2">
                <p><span className="font-medium">Name:</span> {contract?.firstName} {contract?.secondName}</p>
                <p><span className="font-medium">ID:</span> {contract?.contractorID}</p>
                <p><span className="font-medium">Department:</span> {contract?.department}</p>
                <p><span className="font-medium">Job Title:</span> {contract?.jobTitle}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="mt-2 bg-gray-50 p-4 rounded-md border border-gray-100 space-y-2">
                <p><span className="font-medium">Payment Method:</span> {contract?.paymentMethod}</p>
                <p><span className="font-medium">Currency:</span> {contract?.currency}</p>
                {contract?.paymentMethod === 'bank' && (
                  <>
                    <p><span className="font-medium">Bank:</span> {contract?.bankName}</p>
                    <p><span className="font-medium">Account:</span> {contract?.bankAccountNumber}</p>
                  </>
                )}
                {contract?.paymentMethod === 'crypto' && (
                  <p><span className="font-medium">Wallet:</span> {contract?.walletAddress}</p>
                )}
                {contract?.paymentMethod === 'phone' && (
                  <p><span className="font-medium">Phone:</span> {contract?.paymentPhone}</p>
                )}
              </div>
            </div>
          </div>
