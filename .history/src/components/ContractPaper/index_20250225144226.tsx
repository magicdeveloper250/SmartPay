...
const ContractPaper = ({ contract }:{contract:ContractTerms}) => {
      ...
      <div className="text-center py-8 border-b border-gray-200 bg-gray-50">
        ...
        <div className="flex justify-center items-center gap-2">
          <span className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-sm">
            Contract Reference: {contract?.contract.id}
          </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="mt-2 bg-gray-50 p-4 rounded-md border border-gray-100 space-y-2">
                <p><span className="font-medium">Start Date:</span> {formatDate(contract?.contract.startDate || new Date())}</p>
                <p><span className="font-medium">End Date:</span> {formatDate(contract?.contract.endDate || new Date())}</p>
              </div>
            </div>
            <div>
              ...
              <div className="mt-2 bg-gray-50 p-4 rounded-md border border-gray-100 space-y-2">
                ...
                {contract?.contractor.paymentMethod === 'bank' && (
                  <>
                ...
                {contract?.contractor.paymentMethod === 'crypto' && (
                  <p><span className="font-medium">Wallet:</span> {contract?.contractor.walletAddress}</p>
                )}
                {contract?.contractor.paymentMethod === 'phone' && (
                  <p><span className="font-medium">Phone:</span> {contract?.contractor.paymentPhone}</p>
                )}
              </div>
            </div>
          </div>
        ...
        <section className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-2 text-gray-800 bg-gray-50 p-4 rounded-md border border-gray-100 font-medium text-xl">
              ...
              }).format(contract?.contract.salary || 0)}
        ...
        <section className="space-y-4">
          ...
          <div>
            <div className="mt-2 text-gray-800 bg-gray-50 p-6 rounded-md border border-gray-100 min-h-[120px] whitespace-pre-wrap prose">
              {contract?.contract.notes || 'No additional terms or conditions specified.'}
            </div>
          </div>
        </section>
        ...
        <section className="mt-12 pt-6 border-t border-gray-200 text-sm text-gray-600">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p>{new Date(contract?.contract.createdAt || new Date()).toLocaleString()}</p>
            </div>
            <div>
              <p className="font-medium">Last Modified:</p>
              <p>{new Date(contract?.contract.updatedAt || new Date()).toLocaleString()}</p>
            </div>
          </div>
        </section>
      </div>
