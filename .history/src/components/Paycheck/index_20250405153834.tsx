    ...
    <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
      ...
      <div className="bg-blue-700 text-white p-6">
        <div className="grid grid-cols-2">
          <div>
            ...
            <p className="text-sm">{paycheckData.employee?.company?.address}</p>
          ...
          <div className="text-right">
            ...
            <p className="text-md">Payment Date: {formatDate(new Date(paycheckData.mainPayroll?.paymentDate))}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 border-b border-gray-300 bg-gray-50">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p><span className="font-semibold">Name:</span> {paycheckData.employee?.firstName} {paycheckData.employee?.lastName}</p>
            <p><span className="font-semibold">Employee ID:</span> {paycheckData.employee?.employeeId}</p>
            <p><span className="font-semibold">Position:</span> {paycheckData.employee?.position}</p>
          </div>
          <div>
            <p><span className="font-semibold">Department:</span> {paycheckData.employee?.department}</p>
            <p><span className="font-semibold">Employment Type:</span> {paycheckData.employee?.employmentType}</p>
            <p><span className="font-semibold">Hire Date:</span> {paycheckData.employee?.hireDate ? formatDate(new Date(paycheckData.employee.hireDate)) : 'N/A'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 border-b border-gray-300">
        ...
        {paycheckData.taxes && paycheckData.taxes.map((tax, index) => (
          <div className="grid grid-cols-4 gap-2 py-1" key={`tax- {index}`}>
            <div>{tax.taxName}</div>
            <div>{tax.taxType || 'Statutory'}</div>
            <div className="text-center"> {tax.taxAmount.toFixed(2)}</div>
            <div className="text-right">{tax.rate ? ` {tax.rate}%` : 'Variable'}</div>
          </div>
        ))}
          ...
          <div className="bg-white p-4 rounded shadow">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="p-2">
                ...
                <p className="text-lg"> {(totalTaxes || Number(totalTaxes)).toFixed(2)}</p>
              </div>
              <div className="p-2">
                <p className="text-sm font-semibold">YTD Deductions:</p>
                <p className="text-lg"> {(paycheckData.to || totalDeductions).toFixed(2)}</p>
              </div>
              <div className="p-2">
                <p className="text-sm font-semibold">YTD Net:</p>
                <p className="text-lg"> {(paycheckData.ytdNet || Number(paycheckData.netSalary)).toFixed(2)}</p>
              </div>
            </div>
          </div>
        </div>
