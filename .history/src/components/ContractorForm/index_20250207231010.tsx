...
const employeeSchema = z.object({
  ...
  paymentPhone: z.string().regex(/^\+?[1-9]\d{1,14}$/, "Invalid phone number"),
  address: z.string().min(5, "Address must be at least 5 characters"),
  contractorID: z.string().min(1, "Employee ID is required"),
...
type BankType= "none"|"crypto" | "bank"| "phone"
         ...
         <legend className="text-xl font-semibold mb-4 col-span-2">Personal Information</legend>
          ...
          <div>
           ...
           <div className="grid grid-cols-3 md:grid-cols-3 gap-6 mb-6">  
                ...
                <button 
                  onClick={() => setBankType("phone")}
                  className="h-12 border-2 border-gray-200 rounded-lg hover:border-blue-500 hover:bg-blue-50 transition-colors"
                  type="button"
                >
                  Mobile Phone
                </button>
                <input type="hidden" value={banckType}  name="paymentMethod"/>
              </div>
          {banckType=="bank"&&<>
          ...
          {banckType=="phone"&& <div>
           ...
           <fieldset className="mt-8">
           <legend className="text-xl font-semibold mb-4 mt-4">Contract Terms</legend>
