...
export default function ExportControl({ query, page, type }: ExportControlProps) {
  ...
  const handleExport = async (exportType: "csv" | "json" | "db") => {
    ...
    try {
      ...
      if (exportType === "csv") {
        resp = await exportPayrollToCSV(query, page, type);
      } else if (exportType === "db") {
        resp = await type==='employees'?SavePayroll(selectedDate):SaveContractorPayroll(selectedDate);
      } else {
        resp = await exportPayrollToJSON(query, page, type);
      }
    
      if (resp?.error) {
        toast.error(`Export failed: ${resp.error}`);
      } else {
        toast.success(`${type === 'employees' ? 'Employee' : 'Contractor'} payroll exported successfully`);
      }
    } catch (error) {
    ...
    <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 border border-gray-100">
      <div className="flex flex-col space-y-4 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:w-auto" ref={dropdownRef}>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 sm:right-0 left-0 sm:left-auto mt-2 w-full sm:w-72 
                          origin-top-right rounded-lg bg-white shadow-lg 
                ...
                >
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-xs text-gray-500">
                      Save {type === 'employees' ? 'employee' : 'contractor'} payroll data
                    </p>
                  </div>
                </button>
