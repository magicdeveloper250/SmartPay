...
export default async function Page({ tab, query, currentPage }: { query: string; currentPage: number; tab: string }) {
  ...
  <ul className="divide-y divide-gray-300 border border-gray-400 rounded-lg">
      {employees
        .flatMap((employee) => employee.additionalIncomes.map((income) => ({
          id:income.id,
          incomeType: income.income_type,
          amount: income.amount
        })))
        .reduce((acc, income) => {
          const existing = acc.find((i) => i.id=== income.id);
          if (existing) {
            existing.amount += income.amount;
          } else {
            acc.push({ incomeType: income.incomeType, amount: income.amount });
          }
          return acc;
        }, [])
        .map((income, index) => (
  ...
  <TableCell className="border border-gray-400 font-bold bg-blue-100">
    {new Intl.NumberFormat('en-US', { style: 'currency', currency: employees[0].currency }).format(
      employees.reduce((sum, employee) => sum + ((employee.monthlyGross - employee.appliedTaxes.reduce((taxSum, tax) => taxSum + (employee.monthlyGross * tax.tax.rate), 0)) + employee.additionalIncomes.reduce((addSum, income) => addSum + (employee.monthlyGross * income.amount), 0)), 0)
    )}
  </TableCell>
</TableRow>
