...
export default async function Page({ tab, query, currentPage }: { query: string; currentPage: number; tab: string }) {
  ...
  return (
    <div className="overflow-x-auto mt-0">
      ...
      <Table className="min-w-full border-collapse border border-gray-200">
        ...
        <TableBody>
        {tab == "employees" && payroll?.processedEmployees?.length > 0 && (
  <>
  ...
  <ul className="divide-y divide-gray-300 border border-gray-400 rounded-lg">
      { payroll?.totalAdditionalIncomes.map((income, index) => (
        ...
        <li className="border border-green-400 font-bold bg-blue-100">
    {new Intl.NumberFormat('en-US', { style: 'currency', currency: payroll?.currency}).format(
  ...
  <TableCell className="border border-gray-400 bg-red-100 p-0" colSpan={2}>
    <ul className="divide-y divide-gray-300 border border-gray-400 rounded-lg">
      { payroll?.totalTaxes.map((tax, index) => (
          ...
          <li className="border border-red-400 font-bold bg-blue-100">
    {new Intl.NumberFormat('en-US', { style: 'currency', currency: payroll?.currency}).format(
