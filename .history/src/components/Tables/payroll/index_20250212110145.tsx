    ...
    <TableRow className="text-center text-lg font-semibold bg-gray-100">
      ...
      <TableCell className="border border-gray-400 bg-red-100 p-0">
        <ul className="divide-y divide-gray-300 border border-gray-400 rounded-lg">
          {employees
            .flatMap((employee) => employee.appliedTaxes)
            .reduce((acc, tax) => {
              const existing = acc.find((t) => t.tax.name === tax.tax.name);
              if (existing) {
                existing.total += tax.tax.rate * tax.employeeGross;
              } else {
                acc.push({ tax: tax.tax, total: tax.tax.rate * tax.employeeGross });
              }
              return acc;
            }, [])
            .map((tax, index) => (
              <li key={index} className="grid grid-cols-1 text-sm">
                ...
                <span className="p-2 text-right font-medium">
                  {new Intl.NumberFormat('en-US', { style: 'currency', currency: employees[0].currency }).format(tax.total)}
                </span>
              </li>
            ))}
        </ul>
