             ...
             <ul>
              {
                employee.appliedTaxes.map((tax, index)=>{
                  return <li  key={index} className="grid grid-cols-1 gap-1">
                <span>{tax.tax.name}</span><span> {tax.tax.rate}</span> <span>{new Intl.NumberFormat('en-US', { style: 'currency', currency: employee.currency }).format(employee.monthlyGross * tax.tax.rate)}</span>
