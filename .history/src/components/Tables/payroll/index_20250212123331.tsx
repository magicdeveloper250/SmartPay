  ...
  <>
    {employees.map((employee, i) => (
      <TableRow key={employee.id} className="text-center text-base font-medium">
        ...
        <TableCell className="border border-gray-300 bg-red-50 p-0">
          <div className="w-full">
            {employee.appliedTaxes.map((tax, index) => (
              <div key={index} className="grid grid-cols-3 text-sm border-b last:border-b-0 border-gray-200">
                <div className="p-2 border-r border-gray-200">
                  <span className="font-semibold">{tax.tax.name}</span>
                </div>
                <div className="p-2 border-r border-gray-200 text-center">
                  <span className="text-gray-600">{(tax.tax.rate * 100).toFixed(1)}%</span>
                </div>
                <div className="p-2 text-right">
                  <span>{new Intl.NumberFormat('en-US', { 
                    style: 'currency', 
                    currency: employee.currency 
                  }).format(employee.monthlyGross * tax.tax.rate)}</span>
                </div>
              </div>
            ))}
          </div>
