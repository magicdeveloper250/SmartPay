...
export default async function Page({query, currentPage}: {query: string, currentPage: number}) {
  ...
  const { contractors: { processedContractors }, pagination } = await getContractors(session.user.email, query, currentPage);
  ...
  return (
    <>
      <Table>
        ...
        <TableBody>
          {processedContractors?.map((contractor: Contractor, i: number) => (
            <ContractorTableRow
