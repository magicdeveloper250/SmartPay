...
export default async function Page({query, currentPage}: {query: string, currentPage: number}) {
  ...
  const { contractors } = await getContractors(session.user.email, query, currentPage);
  ...
  return (
    <>
      <Table>
        ...
        <TableBody>
          {contractors.processedContractors?.map((contractor: Contractor, i: number) => (
            <ContractorTableRow
      ...
      <div className="mt-5 flex w-full justify-center">
        <Pagination totalPages={contractors.pagination?.totalPages?contractors.pagination.totalPages:0} />
      </div>
    </>
  );
}