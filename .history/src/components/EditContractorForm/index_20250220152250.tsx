 ...
 const onSubmit = (data: contractorSchemaType) => {
  ...
  useEffect(() => {
      if (contractor && contract) {
        ...
        setValue("email", contractor.email); //mandatory
        ...
        setValue("startDate", new Date(contract.startDate).toISOString().split('T')[0] || new Date().toISOString()); 
        setValue("endDate", new Date(contract.endDate).toISOString().split('T')[0] || new Date().toISOString()); 
