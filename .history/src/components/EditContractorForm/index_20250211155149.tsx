...
import { updateContractor } from '@/actions/contractorActions';
 ...
 export default  function EditContractorForm({contractorId, contractId}:{contractorId:string, contractId:string}) {
 ...
 const onSubmit = (data: contractorSchemaType) => {
    startUpdatingTransition(async () => {
      const response = await updateContractor(contractor?.id || data.contractorID,contractId, data)
      if (response?.error) {
        toast.error(response?.error || "update failed");
      } else {
        toast.success("Contractpr information updated successfully");
        reset()
  ...
  useEffect(() => {
      if (contractor) {
        ...
        setValue("startDate", new Date(contractor.startDate).toISOString().split('T')[0]); 
        setValue("jobTitle", contractor.jobTitle);
        setValue("monthlyGross", contractor.monthlyGross);
