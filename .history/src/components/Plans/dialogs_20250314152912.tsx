...
export const OfferFormDialog: React.FC<OfferFormDialogProps> = ({ open, onCancel }) => {
  ...
  const getAllPlans = async () => {
    try {
      const result = await getPlans(); // Define this function if it doesn't exist
  ...
  useEffect(() => {
    Promise.all([getPlanCompanyTiers(), getAllFeatures(), getAllPlans()]);
  }, []);
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    step="0.01"
                    {...register('offer.discountAmount', { valueAsNumber: true })}
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  />
                  {errors.offer?.discountAmount && (
                    <p className="text-red-500 text-xs mt-1">{errors.offer.discountAmount.message}</p>
                  )}
                </div>
