...
const PlanFormDialog: React.FC<PlanFormDialogProps> = ({ 
  ...
  // Use field array for features to properly track form state
  const { fields, append, remove, replace } = useFieldArray({
    control,
    name: "features.features"
  });

  // Get selected feature IDs from the form state
  ...
  const onSubmit: SubmitHandler<PlanFormValues> = (data) => {
    ...
    startTransition(async () => {
      try {
        // Data already has the correct structure - features are in data.features.features
        // Just need to ensure no null descriptions
        const sanitizedData = {
          ...data,
          features: {
            features: data.features.features.map(feature => ({
              ...feature,
        ...
        const result = await addNewPlan(sanitizedData);
  ...
  const handleFeatureSelection = (feature: Feature) => {
    const currentFeatures = getValues("features.features") || [];
    const featureExists = currentFeatures.some(f => f.id === feature.id);
    
    if (featureExists) {
      // Remove feature if already selected
      const updatedFeatures = currentFeatures.filter(f => f.id !== feature.id);
      replace(updatedFeatures);
    } else {
  ...
  const isFeatureSelected = (featureId: string) => {
    return fields.some(field => field.id === featureId);
  };
        ...
        >
          <div className="flex items-center justify-between p-4 border-b">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              ...
              {/* Hidden debug info */}
              <div className="text-xs mb-2 text-gray-500">Form dirty state: {isDirty ? "Dirty" : "Not Dirty"}</div>
                ...
                <div>
                    ...
                    {availableFeatures.length === 0 ? (
                      ...
                      availableFeatures.map(feature => (
                        ...
                        >
                          ...
                          {isFeatureSelected(feature.id) && feature.description && (
                  ...
                  {fields.length > 0 && (
                    <p className="text-sm text-gray-600 mt-2">
                      {fields.length} feature{fields.length !== 1 ? 's' : ''} selected
                    </p>
                  )}
                </div>
