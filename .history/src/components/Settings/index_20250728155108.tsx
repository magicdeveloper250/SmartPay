...
export function TaxesForm({ initialData }: { initialData: TaxValues[] }) {
  ...
  const availableTaxes = Object.values(SupportedTaxes).filter(
    tax => !selectedTaxes.some(t => t.tax.name === tax)
  );
        ...
        <div className="space-y-6">
            ...
            <div key={field.id} className="p-4 border rounded-md bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                <div>
                  ...
                  >
                    {Object.values(SupportedTaxes).map((tax, i) => (
                      <option 
                        key={i} 
                        value={tax}
                        disabled={selectedTaxes.some((t, idx) => 
                          t.tax.name === tax && idx !== index
                        )}
                      >
                        {tax}
                      </option>
          ...
          <div className="flex justify-center">
            <button 
              ...
              disabled={availableTaxes.length === 0}
              className="px-4 py-2 bg-green-800 text-white font-medium rounded-md hover:bg-green-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
...
export function ContributionsForm({ initialData }: { initialData: ContributionValues[] }) {
  ...
  const selectedContributions = watch("contributions") || [];
  const availableContributions = Object.values(SupportedContributions).filter(
    contribution => !selectedContributions.some(c => c.contribution.name === contribution)
  );
        ...
        <div className="space-y-6">
          {fields.map((field, index) => (
            <div key={field.id} className="p-4 border rounded-md bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                <div>
                  <label htmlFor={`contributions.${index}.contribution.name`} className="block text-sm font-medium mb-1 text-gray-700">
                  ...
                  <select
                    ...
                    className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary outline-none"
                  >
                    {Object.values(SupportedContributions).map((contribution, i) => (
                      <option 
                        key={i} 
                        value={contribution}
                        disabled={selectedContributions.some((c, idx) => 
                          c.contribution.name === contribution && idx !== index
                        )}
                      >
                        {contribution}
                      </option>
          ...
          <div className="flex justify-center">
            <button 
              ...
              disabled={availableContributions.length === 0}
              className="px-4 py-2 bg-green-800 text-white font-medium rounded-md hover:bg-green-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
...
export default function SettingsForm({ initialData }: { initialData: SettingsFormValues }) {
  const tabs = [
    ...
    { id: 'payroll-settings', label: 'Payroll Settings' },
     { id: 'departments-settings', label: 'Departments' },
  ...
  const activeTab = useMemo(() => {
    ...
    return tabs.some(t => t.id === tabFromParams) ? tabFromParams : tabs[0].id;
  }, [searchParams]);
  ...
  }, [searchParams, pathname, replace, tabs]);
  ...
  return (
    <div className="w-full mx-auto rounded-lg p-6 bg-white shadow-md">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">
        {(!initialData || 'error' in initialData) ? "Welcome! Let's set up your company settings" : "Company Settings"}
      </h1>
      ...
      {/* Tab Content */}
      <div className="mt-4">
        {activeTab === 'company-profile' && <CompanyProfileForm initialData={initialData} />}
        {activeTab === 'general-settings' && <GeneralSettingsForm initialData={initialData} />}
        {activeTab === 'payroll-settings' && <PayrollSettingsForm initialData={initialData} />}
        {activeTab === 'taxes' && <TaxesForm initialData={initialData.taxes?.map(tax => ({ tax }))} />}
        {activeTab === 'contributions' && <ContributionsForm initialData={initialData.contributions?.map(contribution => ({ contribution }))} />}
      </div>

      {/* Footer */}
      <div className="text-sm text-gray-500 mt-4">
        <p><span className="text-red-800">*</span> indicates required fields</p>
      </div>
    </div>
  );
}