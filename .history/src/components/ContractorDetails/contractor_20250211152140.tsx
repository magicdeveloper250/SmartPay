    ...
    <div className="w-full max-w-7xl mx-auto bg-white rounded-lg shadow-lg p-6 transform transition-all hover:shadow-2xl">
      <div className="w-full flex justify-between items-center mb-6">
        ...
        <div className="w-full grid grid-cols-2 md:grid-cols-2 gap-4 mb-6">
          ...
          <span className="text-gray-600">{new Intl.NumberFormat('en-US', { style: 'currency', currency: contractor.currency }).format(contracts[0].salary) }</span>
          <span className="font-medium text-gray-700">Payment Method:</span>
          <span className="text-gray-600">{contractor.paymentMethod}</span>
          <Link href={`/contractor/internal/${contracts[0].id}/contract`} scroll={false} className='inline-flex p-2 border-dark border-spacing-34'><EyeIcon className='w-5 h-5'/>  View Contract</Link>
        </div>
