...
export default function EmployeeDisplay({employee, benefits, taxes}:{employee:Employee, benefits:EmployeeBenefit[], taxes:Tax[]}) {
  ...
  <div className="w-full grid grid-cols-2 md:grid-cols-2 gap-3 mb-3">
    ...
    <div>
      <div className="w-full flex justify-between items-center mb-3">
        ...
        <button onClick={()=>setShowBenefits(!showBenefits)}><Plus className="w-5 h-5" /></button>
      </div>
      <div>
        ...
        {!showBenefits&&<span>No allowed Benefit Found</span>}
      </div>
  {benefits.length > 0 ? (
    ...
    <div className="mt-4">
      <div className="w-full flex justify-between items-center mb-3">
        ...
        <button onClick={()=>setShowTaxes(!showTaxes)}><Plus className="w-5 h-5" /></button>
      </div>
      {showTaxes&&<select name="" id="">
        <option value="" disabled selected>Select Taxe Type</option>
  ...
  ):<span>No allowed Taxes Found</span>}
