...
import { addEmployeeBenefit, deleteEmployeeBenefit } from '@/actions/employeeActions';
    ...
    <div className="w-full max-w-7xl mx-auto bg-white rounded-lg shadow-lg p-6 transform transition-all hover:shadow-2xl">
      ...
      <div>
          ...
          {showTaxes && (
            <select className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              {AllTaxes.map((tax, index) => (
                <option key={index} className="text-gray-700">{tax.name} [ {new Intl.NumberFormat('en-US', { style: 'currency', currency: employee.currency }).format(tax.min) || "0.00"} - {new Intl.NumberFormat('en-US', { style: 'currency', currency: employee.currency }).format(tax.max ||0) || "0.00"} {tax.rate*100}%]</option>
              ))}
            </select>
          )}
        </div>
        {taxes.length > 0 ? (
          <ul className="list-disc pl-5 text-gray-600">
            {taxes.map((tax, index) => (
              <li key={index} className="mb-2">{AllTaxes.find((Atax)=>Atax.id=tax.taxId)?.name}</li>
            ))}
          </ul>
