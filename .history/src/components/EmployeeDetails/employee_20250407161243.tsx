...
import {
  ...
  addEmployeeTax,
  changeEmployeeDeductionStatus,
  changeEmployeeIncomeStatus,
  deleteEmployee,
...
export default function EmployeeDisplay({
  ...
  const [AllContributions, setContributions] = useState<
    PredefinedContribution[]
  >([]);
  ...
  const [isAddingContribution, startAdddingContributionTransition] =
    useTransition();
  const [isDeletingTax, startDeletingTaxTransition] = useTransition();
  const [isDeletingContribution, startDeletingContributionTransition] =
    useTransition();
  const [isDeletingIncome, startDeletingIncomeTransition] = useTransition();
  const [isChangingIncomeStatus, startChangingStatusTransition] =
    useTransition();
  const [isAddingDeduction, startAdddingDeductionTransition] = useTransition();
  const [isDeletingDeduction, startDeletingDeductionTransition] =
    useTransition();
  const [isChangingDeductionStatus, startChangingDeductionStatusTransition] =
    useTransition();
  ...
  const [loadingDeductionId, setLoadingDeductionId] = useState<string | null>(
    null,
  );
  ...
  const handleAddingContribution = (
    employeeId: string,
    contributionId: string,
  ) => {
    startAdddingContributionTransition(async () => {
      try {
        const response = await addEmployeeContribution(
          employeeId,
          contributionId,
        );
        if ("error" in response) {
  ...
  const handleChanngeIncomeStatus = (
    id: string,
    employeeId: string,
    ...
    startChangingStatusTransition(async () => {
      try {
        const response = await changeEmployeeIncomeStatus(
          id,
          employeeId,
          newStatus,
        );
  ...
  const handleChanngeDeductionStatus = (
    id: string,
    employeeId: string,
    newStatus: PaymentStatus,
  ) => {
    ...
    startChangingDeductionStatusTransition(async () => {
      try {
        const response = await changeEmployeeDeductionStatus(
          id,
          employeeId,
          newStatus,
        );
  ...
  return (
    <div className="mx-auto w-full max-w-7xl p-4">
      <div className="flex flex-col gap-6 lg:flex-row">
        <div className="w-full rounded-lg border border-gray-100 bg-white p-6 shadow-sm lg:w-1/3">
          <div className="mb-6 flex items-center justify-between">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-blue-50 px-4 py-2 transition-colors hover:bg-blue-100"
              scroll={false}
            >
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-1 lg:grid-cols-1">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-500">
                  {formatPaymentFrequency(employee.paymentFrequency)} Salary
                </p>
                <p className="text-base font-semibold text-gray-900">
            ...
            <button
              className="w-full rounded-md bg-red-700 py-2 font-medium text-white hover:bg-red-500"
              onClick={() => setShowDeletDialog(true)}
            >
              Delete
            </button>
          </div>
  ...
  <div className="grid grid-cols-1 gap-6 md:grid-cols-2 auto-rows-auto">
    <div className="break-inside-avoid rounded-lg border border-gray-100 bg-white p-6 shadow-sm h-auto flex flex-col">
      <div className="mb-6 flex items-center justify-between">
        ...
        <PrimaryButton onClick={() => setShowBenefits(!showBenefits)}>
          {!showBenefits ? (
            <Plus className="h-5 w-5" />
          ) : isAddingBenefit ? (
            <Loader className="h-4 w-4 animate-spin text-green" />
          ) : isDeletingBenefit ? (
            <Loader className="h-4 w-4 animate-spin text-red" />
          ) : (
      ...
      <div className="mb-4">
        {showBenefits && (
          <select
            className="border-primary-300 w-full rounded-lg border p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
              handleAddingBenefit(employee.id, e.target.value)
            }
          >
            ...
            {Object.values(BenefitName).map((eBenefit, index) => (
              <option
                key={index}
                className={`text-gray-700 ${benefits.some((benefit) => benefit.benefit == eBenefit) && "bg-gray-6"}`}
                disabled={benefits.some(
                  (benefit) => benefit.benefit == eBenefit,
                )}
              >
                {eBenefit}
              </option>
        ...
        {benefits.length > 0 ? (
          <ul className="list-disc pl-5 text-gray-600">
            {benefits.map((benefit, index) => (
              <div
                className="mb-6 flex w-full justify-between gap-4"
                key={index}
              >
                <li className="mb-2 w-full overflow-x-hidden text-sm">
                  {benefit.benefit}
                </li>
                <SecondaryButton
                  variant="danger"
                  onClick={() =>
                    handleDeletingBenefit(
                      benefit.id,
                      employee.id,
                      benefit.benefit,
                    )
    ...
    <div className="rounded-lg border border-gray-100 bg-white p-6 shadow-sm h-auto flex flex-col">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
        ...
        <Link
          ...
          scroll={false}
          className="hover:bg-primary-600 inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 
      text-white transition-all duration-200"
        >
          {isDeletingIncome ? (
            <Loader className="h-4 w-4 animate-spin text-red" />
          ) : isChangingIncomeStatus ? (
        ...
        {additionalIncomes.length > 0 ? (
          <ul className="list-disc pl-5 text-gray-600">
            {additionalIncomes.map((income, index) => (
              ...
              >
                <li
                  className="mb-4 rounded-lg border border-gray-100 bg-white p-4 shadow-md transition-all duration-200 hover:shadow-lg text-sm"
                  key={index}
                >
                  <div className="flex flex-col space-y-3">
                    ...
                    {income.description && (
                      <div className="border-t border-gray-100 pt-2 text-sm italic text-gray-500">
                        {income.description}
                      </div>
                    )}
                  </div>
      ...
      <div className="mb-4">
        {showTaxes && (
          <select
            ...
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
              handleAddingTax(employee.id, e.target.value)
            }
          >
            <option value="" disabled selected>
              Select Tax
            ...
            {AllTaxes.map((tax, index) => {
              const disabled = taxes.some(
                (etax) => etax.taxId === tax.id,
              );
              return (
                <option
                  key={index}
        ...
        {taxes.length > 0 ? (
          <ul className="space-y-4">
            {taxes.map((tax, index) => (
              <li
                key={index}
                className="flex items-center justify-between rounded-lg border border-gray-100 bg-white p-4 shadow-sm text-sm"
              >
                <span className="font-medium text-gray-900">
                  {AllTaxes.find((Atax) => Atax.id === tax.taxId)?.name ||
                    "Unknown Tax"}
                </span>
                <SecondaryButton
                  variant="danger"
    ...
    <div className="rounded-lg border border-gray-100 bg-white p-6 shadow-sm h-auto flex flex-col">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <BookHeartIcon className="h-5 w-5 text-blue-600" />
          <h2 className="text-lg font-semibold text-gray-900">
            RSSB Contributions
          </h2>
        </div>
      ...
      <div className="mb-4">
        {showContributions && (
          <select
            className="w-full rounded-lg border border-gray-300 p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
              handleAddingContribution(employee.id, e.target.value)
            }
          >
            <option value="" disabled selected>
              Select Contribution
            ...
            {AllContributions.map((contribution, index) => {
              const disabled = contributions.some(
                (eContribution) =>
                  eContribution.contributionId === contribution.id,
              );
              return (
                <option
                  key={index}
      ...
      <div className="flex-grow">
        {contributions.length > 0 ? (
              ...
              >
                <span className="font-medium text-gray-900">
                  {AllContributions.find(
                    (AContribution) =>
                      AContribution.id === contribution.contributionId,
                  )?.name || "Unknown Contribution"}
                </span>
                <SecondaryButton
                  variant="danger"
                  onClick={() =>
                    handleDeletingContribution(
                      contribution.id,
                      employee.id,
                    )
                  }
          ...
          <span className="text-gray-500">
            No assigned Contributions Found
          </span>
        )}
      </div>
    </div>
    <div className="rounded-lg border border-gray-100 bg-white p-6 shadow-sm h-auto flex flex-col">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Wallet2 className="h-5 w-5 text-red-600" />
          <h2 className="text-lg font-semibold text-gray-900">
            Deductions
          </h2>
        </div>
          ...
          <ul className="list-disc pl-5 text-gray-600">
            {deductions.map((deduction, index) => (
              <div
                ...
                key={index}
              >
                <li className="mb-4 rounded-lg border border-gray-100 bg-white p-4 shadow-md transition-all duration-200 hover:shadow-lg text-sm">
                  <div className="flex flex-col space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mx-auto flex size-12 shrink-0 items-center justify-center rounded-full bg-red-100 sm:mx-0 sm:size-10">
                    <AlertCircle
                      aria-hidden="true"
                      className="size-6 bg-red-700 text-white"
                    />
                  </div>
                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        ...
                        All of the data will be permanently removed. This action
                        cannot be undone.
                      </p>
                    </div>
                  </div>
              ...
              <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                ...
                >
                  {isDeletingEmployee && <Loader2 className="animate-spin" />}{" "}
                  Delete
                </button>
                <button
                  type="button"
...
function PrimaryButton({
  ...
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className={`hover:bg-primary-600 disabled:bg-primary-300 inline-flex items-center gap-2 rounded-lg bg-primary px-4 
        py-2 text-white transition-all duration-200 
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 rounded-lg px-4 py-2
        ${variants[variant]} transition-all duration-200 focus:outline-none
        focus:ring-2 focus:ring-offset-2 disabled:opacity-50 ${className}`}
    >
      {loading && <Loader className="h-4 w-4 animate-spin" />}
      {children}
    </button>
