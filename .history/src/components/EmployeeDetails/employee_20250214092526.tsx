...
import { addEmployeeBenefit, addEmployeeTax, changeEmployeeIncomeStatus, deleteEmployeeBenefit, deleteEmployeeIncome, deleteEmployeeTax } from '@/actions/employeeActions';
...
export default function EmployeeDisplay({ employee, benefits, taxes,additionalIncomes }: { employee: Employee, benefits: EmployeeBenefit[], taxes: AppliedTax[], additionalIncomes:AdditionalIncome[] }) {
  ...
  const[isPayingIncome, startPayingIncomeTransition]= useTransition() 
    ...
    <div className="w-full max-w-7xl mx-auto bg-white rounded-lg shadow-lg p-6 transform transition-all hover:shadow-2xl">
      ...
      <div className="w-full flex justify-between items-center mb-4">
          ...
          > 
              ...
              {isDeletingIncome || isChangingIncomeStatus|| isPayingIncome ? (
                <Loader className="w-5 h-5  text-white"  />
              ) : (
                <Plus className="w-5 h-5  text-white" />
              )}
                 ...
                 {income.payment_status === PaymentStatus.Pending && (
                   ...
                   {(income.payment_status == PaymentStatus.Cancelled) && (
                    <button 
                      ...
                      className="px-4 py-2 text-sm font-medium   bg-amber-50 text-amber-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors duration-200">
                      Pending
                    </button>
                  )}
