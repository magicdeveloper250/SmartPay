...
export default function EmployeeDisplay({ employee, benefits, taxes,additionalIncomes, deductions }: { employee: Employee, benefits: EmployeeBenefit[], taxes: AppliedTax[], additionalIncomes:AdditionalIncome[], deductions:Deduction[] }) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto bg-white rounded-lg shadow-lg p-8 space-y-8">
      <div className="flex justify-between items-center border-b pb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{employee.firstName} {employee.secondName}</h1>
          <p className="text-gray-500 mt-1">Employee ID: {employee.id}</p>
        </div>
        <Link
          href={`/employee/internal/${employee.id}/edit`}
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 transition-all duration-200"
          scroll={false}
        >
      ...
      <div className="bg-gray-50 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <InfoCard label="Phone" value={employee.phoneNumber} />
          <InfoCard 
            label="Salary" 
          ...
          />
          <InfoCard label="Payment Method" value={employee.paymentMethod} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            ...
            className={`transition-all duration-200 px-4 py-2 rounded-lg flex items-center gap-2
              ${showBenefits 
          ...
          >
            {isAddingBenefit || isDeletingBenefit ? (
              ...
              <>
                {showBenefits ? <MinusIcon className="w-5 h-5" /> : <Plus className="w-5 h-5" />}
                <span>{showBenefits ? 'Close' : 'Add Benefit'}</span>
              </>
            )}
          </button>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link
            ...
            scroll={false}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white hover:bg-primary-600 transition-all duration-200"
          >
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-gray-600 text-sm">
                    {new Date(income.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                  </span>
                </div>
