...
import { Plus, PencilIcon, MinusIcon,  Loader, Building2, Phone, Wallet2, CreditCard, BadgeCheck, BookHeartIcon } from "lucide-react";
...
import { addEmployeeBenefit, addEmployeeContribution, addEmployeeTax, changeEmployeeDeductionStatus, changeEmployeeIncomeStatus, deleteEmployeeBenefit, deleteEmployeeContribution, deleteEmployeeDeduction, deleteEmployeeIncome, deleteEmployeeTax } from '@/actions/employeeActions';
...
export default function EmployeeDisplay({ employee, benefits, taxes,contributions, additionalIncomes, deductions }: { employee: Employee, benefits: EmployeeBenefit[], taxes: AppliedTax[], contributions:AppliedContribution[],additionalIncomes:AdditionalIncome[], deductions:Deduction[] }) {
  ...
  console.log("deleying tax: ", isDeletingTax)
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
        ...
        { additionalIncomes.length > 0 ? (
         ...
         <li className="mb-4 p-4 bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200 border border-gray-100"  key={index}>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
            { isAddingTax &&  <Loader className="w-4 h-4 animate-spin text-green-800" />
           
              
            }
             {isDeletingTax && <div className='relative top-0 w-full h-full right-0 bg-black z-100'><Loader className="w-4 h-4 animate-spin" /></div>}
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                <BadgeCheck className="w-5 h-5 text-blue-600" />
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white 
                  hover:bg-primary-600 transition-all duration-200"
              >
                {!showTaxes ?  <Plus className="w-5 h-5" /> : (
                  <>
                    <MinusIcon className="w-5 h-5" />
        ...
        {taxes.length > 0 ? (
          ...
          <span className="text-gray-500">No assigned Taxes Found</span>
            ...
            {/* Section 4 - Deductions (Bottom Right) */}
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
          ...
          >
            {!showContributions ? (
              isAddingContribution || isDeletingContribution ? (
                <Loader className="w-4 h-4 animate-spin" />
              ) : (
                <>
                  <Plus className="w-5 h-5" />
