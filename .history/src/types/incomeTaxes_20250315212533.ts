...
function calculateRwandaTaxes({
  ...
  if (adjustedTaxableIncome <= LOWER_BRACKET) { 
    lowerBracketTax = 0; 
    } else if ((adjustedTaxableIncome > LOWER_BRACKET) && (taxableAnnualIncome <= MIDDLE_BRACKET)) { 
        lowerBracketTax = LOWER_RATE * (adjustedTaxableIncome- LOWER_BRACKET); 
    } 
    else if ((adjustedTaxableIncome > MIDDLE_BRACKET) && (adjustedTaxableIncome <= UPPER_BRACKET)) { 
        middleBracketTax = MIDDLE_RATE* (adjustedTaxableIncome - MIDDLE_BRACKET)+ (LOWER_RATE*(MIDDLE_BRACKET-LOWER_BRACKET)); 
    }
    else if (adjustedTaxableIncome > UPPER_BRACKET){
    upperBracketTax = UPPER_RATE * (adjustedTaxableIncome - UPPER_BRACKET)+ (MIDDLE_BRACKET*(UPPER_BRACKET-MIDDLE_BRACKET))+ (LOWER_RATE*(MIDDLE_BRACKET-LOWER_BRACKET));
}
