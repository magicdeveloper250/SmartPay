...
import { calculateRwandaIncomeTax, displayRwandaIncomeTax } from "@/types/incomeTaxes";
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const processedPayrollEmployees = employees.map((employee) => {
        ...
        const totalTaxes = employee.appliedTaxes.reduce((sum, tax) =>{
...
export async function getPayrollById(payrollId: string) {
  ...
  try {
    ...
    const customizedPayroll= {
      ...payroll,
      payrolls:payroll.payrolls.map(payroll=>{
        return {...payroll, taxes:payroll.taxes.map((tax, idx) => {
          const taxName = tax?.tax?.name;
          const currentTaxYear = getCurrentSupportedTaxYear(); 
          let taxAmount=0;
          let taxDescription="";
        switch(taxName){
          case  SupportedTaxes.PIT:
             ...
             taxDescription=displayRwandaIncomeTax({taxYear:currentTaxYear, salary:payroll.salary})
        }
        return {taxAmount, taxDescription, taxName}
        })}
      })
      
    }
    return customizedPayroll;  
  } catch (error) {
    return handleActionsPrismaError(error);
  }
}
