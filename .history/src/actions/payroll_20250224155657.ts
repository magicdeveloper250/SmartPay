    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const processedPayrollContractors = contractors.map((contractor) => {
        ...
        const totalTaxes = contractor.appliedTaxes.reduce(
          (sum, tax) => sum + (currentContract.salary * tax.tax.rate),
          0
        );
      ...
      return {
        message: "Contractor payroll saved successfully"
      };
    }, {
...
export async function getPayrollHistory(
  ...
  try {
    ...
    const payrolls = await prisma.mainPayroll.findMany({
      where:{
        status:status===PaymentStatus.All ? {in:[PaymentStatus.Pending,PaymentStatus.Paid,PaymentStatus.Cancelled,PaymentStatus.Failed]} : status,
        payrollType:payrollType===PayrollType.MIXED ? {in:[PayrollType.EMPLOYEE,PayrollType.CONTRACTOR]} : payrollType,
        companyId:company.id
      },
...
export async function getPayrollById(payrollId: string, payrollType:PayrollType) {

  try {
    const payroll = await prisma.mainPayroll.findUnique({
      where: { id: payrollId, payrollType:payrollType },
      include:{
        payrolls:{
          include:{
            employee:payrollType===PayrollType.EMPLOYEE ? true : false,
            contractor:payrollType===PayrollType.CONTRACTOR ? true : false,
            additionalIncomes:payrollType===PayrollType.EMPLOYEE ? true : false,
            benefits:payrollType===PayrollType.EMPLOYEE ? true : false,
            deductions:payrollType===PayrollType.EMPLOYEE ? true : false,
            taxes:payrollType===PayrollType.EMPLOYEE ? true : false
          }
        },
    }});
console.log("payroll",payroll?.payrolls);
    if (!payroll) {
      return { error: "payroll not found" };
    }

    return {payroll:payroll, payrollType};  
  } catch (error) {
    return handleActionsPrismaError(error);
  }
}
