...
export async function getCompanySettings() {
  try {
    ...
    return {
      ...
      contributions: user.company.predefinedContribution || [],
  ...
  try {
    ...
    if (user.company.predefinedTaxes) {
      for (const tax of data.taxes) {
        const existingTax = user.company.predefinedTaxes.find(t => t.id === tax.tax.id || t.name === tax.tax.name);
  ...
  try {
    ...
    if (user.company.predefinedContribution) {
      for (const contribution of data.contributions) {
        const existingContribution = user.company.predefinedContribution.find(c => c.id === contribution.contribution.id || c.name== contribution.contribution.name);
