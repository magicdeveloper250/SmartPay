...
export default function OnBoarding({ setMode, mode }: Props) {
  return (
    <div className="w-full max-w-7xl mx-auto bg-white rounded-lg shadow-lg p-6">
      ...
      {mode === "contractor" && <p>Contractor onboarding coming soon!</p>}

      {/* Fallback for unexpected mode */}
      {!["none", "single", "multiple", "contractor"].includes(mode) && (
        <p>Invalid mode: {mode}</p>
      )}
    </div>
  );
}
