...
function getFieldName(target: string | undefined): string {
  ...
  const field = target.split("_")[1] || target[0];
...
function getModelName(target: string | undefined): string {
  ...
  return target.split("_")[0].toLowerCase();
}
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    switch (error.code) {
      case 'P2002': {
        ...
        const field = getFieldName(error.meta?.target);
        const model = getModelName(error.meta?.target);
