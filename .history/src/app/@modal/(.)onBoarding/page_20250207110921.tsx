...
const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
  ...
  try {
    if (mode === "multiple" && file) {
      // Your existing multiple employees code...
    } else {
      const formData = new FormData(e.currentTarget);
      const formValues = Object.fromEntries(formData);
      
      // Prepare the payload based on bank type
      const basePayload = {
        ...
        monthlyGross: Number(formValues.monthlyGross),
        currency: formValues.currency,
        department: formValues.department,
      };

      // Add bank or crypto details based on selection
      let payloadWithPaymentDetails;
      if (banckType === "bank") {
        payloadWithPaymentDetails = {
          ...basePayload,
        ...
        // If no payment method selected, set all payment fields to null
        payloadWithPaymentDetails = {
          ...basePayload,
          walletAddress: null,
          bankName: null,
      ...
      console.log('Sending payload:', payloadWithPaymentDetails);
      ...
      const response = await fetch("/api/employee", {
        ...
        body: JSON.stringify(payloadWithPaymentDetails),
      });
      ...
      setMode("none"); // Reset form after successful submission
    }
  } catch (err: any) {
