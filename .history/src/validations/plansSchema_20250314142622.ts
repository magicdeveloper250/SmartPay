...
const OfferSchema = z.object({
  ...
  discountAmount: z.number().min(0, "Discount amount must be at least 0.").optional(),
  discountPercentage: z.number().min(0, "Discount percentage must be at least 0.").optional(),

  startDate: z.coerce.date(),
  endDate: z.coerce.date().optional(),
 
  includedPlans: z.array(z.string()).nonempty("At least one plan must be selected"),
  ...
  (data) => !(data.discountAmount !== undefined && data.discountPercentage !== undefined),
  {
    message: "You cannot provide both discountAmount and discountPercentage. Choose one.",
    path: ["discountAmount", "discountPercentage"],
  }
);
