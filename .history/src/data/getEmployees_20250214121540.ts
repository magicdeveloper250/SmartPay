...
export async function getPayrollEmployees(
  ...
  const processedEmployees = employees.map((employee) => {
    ...
    const netSalary = employee.monthlyGross - totalTaxes + totalAdditions;
    ...
    .reduce((acc: { id: string; name: string; rate: number; amount: number }[], tax) => {
       const existing = acc.find((i) => i.id === tax.id);
      if (existing) {
    ...
    const totalAdditonalIncomesAmount= totalTaxes.reduce((sum, income) => sum + income.amount, 0)
    const currency= "RWF"
    const payroll= {processedEmployees, totalMonthlyGross, totalAdditionalIncomes, totalTaxes, totalNetSalary, totalAdditonalIncomesAmount, totalTaxesAmount, currency}
    return id
      ? { employee: employees[0] || null }  
      : {
           payroll: payroll,
