...
data = {
    ...
    "bankAccountNumber": [fake.unique.iban() if np.random.rand() > 0.3 else None for _ in range(num_employees)],
    "swiftCode": [fake.swift11() if np.random.rand() > 0.3 else None for _ in range(num_employees)],
    "Domicile": [fake.country() if np.random.rand() > 0.3 else None for _ in range(num_employees)],
